...
iis = '../results/insertion-indices/gamma_not-trimmed'
results = '../results/biases/check-biases_not-trimmed'
makedir(results)
...
for filename in list_of_files:
    ...
    with open('{0}/{1}'.format(iis, filename), 'r') as iifile:
        for line in iifile:
            cells = line.split()
            iis_dict[cells[0]] = float(cells[1])
            ...
            for line in sequencefile:
                if line.startswith('>'):
                    if gene_name != '':
                        ...
                        tofile.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\n'.format(gene_name, iis_dict[gene_name],
